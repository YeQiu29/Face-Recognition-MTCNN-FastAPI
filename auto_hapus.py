...
async def upload_images(nik: str = Form(...), nama: str = Form(...), files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        success = preprocess_and_train(nik, nama)

        return JSONResponse(content={"success": success, "message": f"Successfully uploaded {len(files)} images for NIK {nik}, Name {nama}"})
    except Exception as e:
...
@app.delete("/hapus/")
async def hapus_data(nik_hapus: str = Form(...), nama: str = Form(...)):
    try:
        train_path = os.path.join(train_img, nik_hapus)
        aligned_path = os.path.join(aligned_img, nik_hapus)

        # Delete the directories if they exist
        if os.path.exists(train_path):
            shutil.rmtree(train_path)
            logging.info(f"Deleted train path: {train_path}")

        if os.path.exists(aligned_path):
            shutil.rmtree(aligned_path)
            logging.info(f"Deleted aligned path: {aligned_path}")

        # Preprocess and train again after deletion
        try:
            preprocess_result = subprocess.run(['python3', 'data_preprocess.py'], check=True, capture_output=True, text=True)
            logging.info(f"Preprocessing output: {preprocess_result.stdout}")

            train_result = subprocess.run(['python3', 'train_main.py'], check=True, capture_output=True, text=True)
            logging.info(f"Training output: {train_result.stdout}")

            reload_classifier()
            update_nik_name_mapping()

            return JSONResponse(content={"success": True, "message": f"Successfully deleted data for NIK {nik_hapus}, Name {nama} and retrained the model", "training_success": True})
        except subprocess.CalledProcessError as e:
            logging.error(f"Training failed after deletion: {e}")
            logging.error(f"Training stdout: {e.stdout}")
            logging.error(f"Training stderr: {e.stderr}")
            return JSONResponse(status_code=500, content={"success": False, "message": f"Training failed after deletion: {e.stderr}", "training_success": False})
    except Exception as e:
        logging.error(f"Error in hapus endpoint: {str(e)}")
        return JSONResponse(status_code=500, content={"success": False, "message": f"An error occurred: {str(e)}"})
